...
class ResultActivity : BaseActivity() {
    ...
    private fun calculateInterest(info: LoanSingleInfo) {
        ...
        for (item in 1..monthCounts) {
            ...
            datePayList.add("${item}${if(language== com.twx.module_tool.fangdai.Language.zh_CN) "期" else " phase"}")
        }

        val firstMonth = info.firstRefundDate.let {
            if(language== com.twx.module_tool.fangdai.Language.zh_CN) {
...
//                    }?.name?:""
    ...
    private fun calculateCombineInterest(info: LoanCombineInfo) {
        ...
        for (i in 1..monthCounts) {
            ...
            datePayList.add("${i}${if(language== com.twx.module_tool.fangdai.Language.zh_CN) "期" else "phase"}")
        }
        //商业贷款月供
