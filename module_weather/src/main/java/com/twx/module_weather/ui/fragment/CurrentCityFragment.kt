...
import com.twx.module_weather.utils.*
                    ...
                    it.msg.day24Msg?.data?.hourly?.apply {
                        ...
                        currentColor =
                            if (hourlyBean.hour.toInt() in 6..18) WeatherUtils.selectDayIcon(
                                hourlyBean.iconDay
                            )[com.twx.module_weather.utils.Constants.MJ_COLOR]!! else WeatherUtils.selectNightIcon(
                                hourlyBean.iconNight
                            )[com.twx.module_weather.utils.Constants.MJ_COLOR]!!

                        val hour = Integer.valueOf(hourlyBean.hour)
                        if (hour in 6..18) {
                            ivTopBg.setImageResource(WeatherUtils.selectDayIcon(hourlyBean.iconDay)[com.twx.module_weather.utils.Constants.MJ_BG]!!)
                            ivIconWeather.setImageResource(WeatherUtils.selectDayIcon(hourlyBean.iconDay)[com.twx.module_weather.utils.Constants.MJ_LAGER_ICON]!!)
                        } else {
                            ivTopBg.setImageResource(WeatherUtils.selectNightIcon(hourlyBean.iconNight)[com.twx.module_weather.utils.Constants.MJ_BG]!!)
                            ivIconWeather.setImageResource(WeatherUtils.selectNightIcon(hourlyBean.iconNight)[com.twx.module_weather.utils.Constants.MJ_LAGER_ICON]!!)
                        }
                        if (size > 0) {
        ...
        binding?.apply {
            tvCheckWea.setOnClickListener {
                toOtherActivity<AirActivity>(activity) {
                    mWeatherInfo?.let {
                        putExtra(
                            com.twx.module_weather.utils.Constants.WEATHER_INFO,
                            Gson().toJson(it)
                        )
                    }
            ...
            rlHuangliInclude.setOnClickListener {
                toOtherActivity<HuangLiActivity>(activity) {
                    huangLiData?.let {
                     putExtra(com.twx.module_weather.utils.Constants.HUANG_LI_DATA,Gson().toJson(it))
                    }
                }
            }
